...
CBaseMesh<CVertex, CEdge, CFace, CHalfEdge>::~CBaseMesh()
{
    ...
    for (auto fiter = m_faces.begin(); fiter != m_faces.end(); fiter++)
    {
        ...
        for (auto hiter = hes.begin(); hiter != hes.end(); hiter++)
        {
            CHalfEdge * pH = *hiter;
            delete pH;
        }
    ...
    while (f.getline(cmd, 1024))
    {
        ...
        if (token == "f")
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                std::vector<std::string> scorner;
                strutil::split(token, scorner, "/");
                ...
                switch (vtype)
                {
                    case 1: // P
                    {
                        int vid = strutil::parseString<int>(scorner[0]);
                        v[i] = m_map_vert[vid];
                        u[i] = CPoint2(0, 0);
                        n[i] = CPoint(0, 0, 0);
                        break;
                    ...
                    case 2: // P/T
                    {
                        int vid = strutil::parseString<int>(scorner[0]);
                        v[i] = m_map_vert[vid];
                        int uid = strutil::parseString<int>(scorner[1]);
                        u[i] = uvs[uid - 1];
                        n[i] = CPoint(0, 0, 0);
                        break;
                    }
                    case 3: // P//N
                    {
                        int vid = strutil::parseString<int>(scorner[0]);
                        v[i] = m_map_vert[vid];
                        int nid = strutil::parseString<int>(scorner[2]);
                        n[i] = normals[nid - 1];
                        u[i] = CPoint2(0, 0);
                        break;
                    }
            ...
            CFace * f = createFace(v, fid++);
            CHalfEdge* h = (CHalfEdge*)f->halfedge();
            h = (CHalfEdge*)h->he_next();
            for (int k = 0; k < 3; k++) {
                h->normal() = n[k];
                h->uv() = u[k];
                h = (CHalfEdge*)h->he_next();
...
CEdge * CBaseMesh<CVertex, CEdge, CFace, CHalfEdge>::createEdge(tVertex  v1, tVertex  v2)
{
    ...
    for (auto te = ledges.begin(); te != ledges.end(); te++)
    {
        CEdge	  * pE = *te;
...
inline CEdge * CBaseMesh<CVertex, CEdge, CFace, CHalfEdge>::vertexEdge(tVertex  v0, tVertex  v1)
{
    ...
    for (auto eiter = ledges.begin(); eiter != ledges.end(); eiter++)
    {
        CEdge * pE = *eiter;
...
void CBaseMesh<CVertex, CEdge, CFace, CHalfEdge>::read_m(const char * input)
{
    ...
    for (auto viter = dangling_verts.begin(); viter != dangling_verts.end(); ++viter)
    {
        CVertex * v = *viter;
        m_verts.remove(v);
        delete v;
...
void CBaseMesh<CVertex, CEdge, CFace, CHalfEdge>::deleteFace(tFace  pFace)
{
    ...
    auto fiter = m_map_face.find(pFace->id());
    ...
    //modified by Wei CHEN, 2018-2-13
    for each (auto pV in isolated_verts)
    {
        //std::map<int, CVertex*>::iterator viter = m_map_vert.find(pV->id());
        auto viter = m_map_vert.find(pV->id());
